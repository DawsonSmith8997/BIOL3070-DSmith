...
library(tidyr)
library(ggplot2)
library(maps)      # Needed for the borders() layer
...
ggplot(hm, aes(long, lat)) +
  ...
  # Overlay sampling sites
  geom_point(
    aes(size = pmax(value, 0.0001)),
    alpha = 0.5,
    shape = 21,
    stroke = 0.2,
    fill = NA,
  ...
  scale_fill_viridis_c(option = "C", direction = -1, name = "Hotspot Intensity") +
  guides(color = "none") +
  theme_minimal(base_size = 11) +
  labs(
    x = NULL,
    y = NULL,
    title = "Hotspot Maps for WNV-positive Pools and House Finch Blood Meals",
    subtitle = "Density estimates across sampling sites in Utah"
  )
